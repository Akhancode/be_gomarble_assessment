...
async function scrapeByCssSelector(page, selectors) {
  ...
  const reviewsData = await page.evaluate(() => {
    ...
    const selectors = {
      reviewAll: ".yotpo-reviews-container",
      review: ".yotpo-review",
      title: ".yotpo-review-title",
      body: ".yotpo-review-content .yotpo-read-more-text",
      rating: ".yotpo-review-star-rating",
...
async function fetchHTML(url) {
  ...
  try {
    ...
    if (reviewData.length == 0) {
      ...
      const cssSelectors = await gemini_prompt(promptForFindingSelectors_2);
